                    ...
                    if (myActiveApplicationCount < 10) {
                      Application.countDocuments({
                        ...
                        if (acceptedJobs === 0) {
                         ...
                         function getScore(){
                           return Job.findOne({ _id: jobId }).then((job) => {
                                jobDescription=job.jobDescription;
                            }).then(()=>{
                                 return  JobApplicant.findOne({userId: user._id}).then((applicant)=>{
                                resume=applicant.resume;
                            })
                           }).then(async ()=>{
                             formData.append('resume', resume);
                             formData.append('description',jobDescription)
                             await axios.post('https://resumescore.onrender.com/api/v1/getResumeScore', formData, {headers: {'Content-Type': 'multipart/form-data'}}).then((res)=>{
                              score=res.data.similarity_score;
                             });
                             return score;
                            })
                          }
